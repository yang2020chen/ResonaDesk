...
export const UploadZone: React.FC<UploadZoneProps> = ({ onSelectFile, disabled }) => {
  ...
  const handlePickNativeFile = async () => {
    ...
    if (isElectronEnv()) {
      try {
        const selected = await (window as any).electronAPI?.openFileDialog?.();
        if (selected && typeof selected === 'string') {
          const name = selected.split('\\').pop()?.split('/').pop() || selected;
          setSelectedFileName(name);
          onSelectFile(selected, { name, size: 0 });
          return;
        }
      } catch (e) {
        console.warn('Electron open dialog error:', e);
      }
    }
  };

  const processFile = async (file: File) => {
    // Mode A: Direct desktop path (Electron / native container)
    ...
    // Mode B: High-speed stream upload fallback (Web)
    setIsUploading(true);
    try {
  ...
  return (
    <div
      onClick={isElectronEnv() ? handlePickNativeFile : undefined}
    ...
    >
      ...
      />
      <label htmlFor={isElectronEnv() ? undefined : 'fileInput'} className="cursor-pointer block">
