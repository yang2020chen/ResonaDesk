...
export const UploadZone: React.FC<UploadZoneProps> = ({ onSelectFile, disabled }) => {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className={`group relative border-2 border-dashed rounded-2xl p-8 text-center transition-all duration-200 cursor-pointer overflow-hidden ${
        dragOver
    ...
    >
      {/* 
        Direct Full-Overlay Native Input:
        Stretched across 100% of the card area with opacity-0.
        Clicks directly hit the physical HTML input element, 100% trusted by WebKit security policies.
      */}
      <input
        type="file"
        id="fileInput"
        ...
        accept="audio/*,video/*,.mp4,.mkv,.mov,.avi,.mp3,.wav,.m4a,.flac,.aac,.ogg,.wma"
        onClick={(e) => {
